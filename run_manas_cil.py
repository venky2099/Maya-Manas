...
def run_manas_cil(seed: int = 42, a_manas: float = A_MANAS):
    print("MayaNexusVS2026NLL_Bengaluru_Narasimha")
    set_seed(seed)
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print(f"Device: {device} | Seed: {seed} | A_manas: {a_manas}")

    model         = MayaManasNet(use_orthogonal_head=False, a_manas=a_manas).to(device)
    ...
    for task_id in range(NUM_TASKS):
        ...
        if task_id > 0:
            ...
            if moha_mask.any():
                ...
                print(f"  [Moha release: {moha_mask.float().mean()*100:.2f}% synapses freed]")
        ...
        for epoch in range(EPOCHS_PER_TASK):
            ...
            for batch_idx, (images, labels) in enumerate(tqdm(
                ...
                # Peak-aligned activity from O-LIF -- Manas gate output
                peak_active = model.get_fc1_peak_active()   # [FC1_SIZE] bool
                ...
                with torch.no_grad():
                    ...
                    # Manas-GANE intersection mask
                    # Synapses that are BOTH Viveka-consistent AND Manas-peak-aligned
                ...
                with torch.no_grad():
                    ...
                    # Manas-GANE: amplify Vairagya gain on intersection synapses
                    ...
                    # Update Manas consistency tracker
            ...
            print(f"    Loss: {epoch_loss/len(train_loader):.4f} | "
                  ...
                  f"Samskara: {chitta.mean_samskara():.3f} | "
    ...
    print(f"\n{'='*50}")
    print(f"  Maya-Manas CIL | seed={seed} | A_manas={a_manas}")
    print(f"  AA  : {final['AA']}%")
